...
class PizzaZine {
private:
    vector<Location> PizzaZineVector;
...
Location& PizzaZine::get(size_t c) {
    return PizzaZineVector[c];
}

Location& PizzaZine::first() {
    return PizzaZineVector[0];
}

Location& PizzaZine::last() {
    return PizzaZineVector[PizzaZineVector.size()-1];
}

Location& PizzaZine::operator[](size_t c) {
    return PizzaZineVector[c];
}

void PizzaZine::add(Location new_location) {
    PizzaZineVector.push_back( new_location );
}
